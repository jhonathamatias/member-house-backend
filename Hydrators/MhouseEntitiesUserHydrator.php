...
class MhouseEntitiesUserHydrator implements HydratorInterface
{
    ...
    public function hydrate(object $document, array $data, array $hints = array()): array
    {
        ...
        /** @ReferenceOne */
        if (isset($data['login'])) {
            $reference = $data['login'];

            if ($this->class->fieldMappings['login']['storeAs'] !== ClassMetadata::REFERENCE_STORE_AS_ID && ! is_array($reference)) {
                throw HydratorException::associationTypeMismatch('Mhouse\Entities\User', 'login', 'array', gettype($reference));
            }

            $className = $this->unitOfWork->getClassNameForAssociation($this->class->fieldMappings['login'], $reference);
            $identifier = ClassMetadata::getReferenceId($reference, $this->class->fieldMappings['login']['storeAs']);
